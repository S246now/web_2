...
/* usar Usuarios, nos permitirá acceder a MongoDB Database */
...
export async function getUsuario(req,res){/* For usuarioId - Single User */
    try {
        const {userId} = req.query;

        if(userId){
            /* const user = await Usuarios.findById(userId); */
            const user = await Usuarios.findById(ObjectId(userId));
